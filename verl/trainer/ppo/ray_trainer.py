...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, advantage='positive', positive_advantage_weight=1, max_reward=1.0, add_max_reward_num=1):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
    elif adv_estimator == AdvantageEstimator.NGRPO:
        token_level_rewards = data.batch['token_level_rewards']
        index = data.non_tensor_batch['uid']
        responses = data.batch['responses']
        response_length = responses.size(-1)
        attention_mask = data.batch['attention_mask']
        response_mask = attention_mask[:, -response_length:]
        advantages, returns = core_algos.compute_ngrpo_outcome_advantage(token_level_rewards=token_level_rewards,
                                                                        eos_mask=response_mask,
                                                                        index=index,
                                                                        max_reward=max_reward,
                                                                        add_max_reward_num=add_max_reward_num)
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
...
class RayPPOTrainer(object):
    ...
    def __init__(self,
        ...
        if self.config.algorithm.adv_estimator == AdvantageEstimator.GAE:
            self.use_critic = True
                ...
                AdvantageEstimator.RLOO, AdvantageEstimator.PSR_NSR, AdvantageEstimator.NGRPO
        ]:
    ...
    def fit(self):
        ...
        while epoch < self.config.trainer.total_epochs:
            ...
            if remained_batch is not None:
                collected_batches.append(remained_batch)
                remained_batch = None
            while len(collected_batches) == 0 or DataProto.concat(collected_batches).batch.batch_size[0] < target_batch_size:
                if len(collected_batches) != 0:
                    print(f"current batch number is {DataProto.concat(collected_batches).batch.batch_size[0]}, ready to continue generating.")
                else:
                    print("current batch number is 0, ready to continue generating.")
                try:
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        if self.config.reward_model.reward_manager == "dapo":
                        ...
                        if self.config.algorithm.filter_groups.enable:
                            filter_all_correct_list = []
                            filter_all_incorrect_list = []
                            filter_all_list = []
                            for i in range(bsz):
                                uid = batch.non_tensor_batch['uid'][i]
                                if id2correctness[uid] == 1.0:
                                    filter_all_correct_list.append(False)
                                    filter_all_incorrect_list.append(True)
                                    filter_all_list.append(False)
                                elif id2correctness[uid] == 0.0:
                                    filter_all_correct_list.append(True)
                                    filter_all_incorrect_list.append(False)
                                    filter_all_list.append(False)
                                else:
                                    filter_all_correct_list.append(True)
                                    filter_all_incorrect_list.append(True)
                                    filter_all_list.append(True)
                            if self.config.algorithm.filter_groups.strategy == "all":
                                print("filter_strategy is all")
                                filter_idx = torch.tensor(filter_all_list, dtype=torch.bool)
                            elif self.config.algorithm.filter_groups.strategy == "all_correct":
                                print("filter_strategy is all_correct")
                                filter_idx = torch.tensor(filter_all_correct_list, dtype=torch.bool)
                            elif self.config.algorithm.filter_groups.strategy == "all_incorrect":
                                print("filter_strategy is all_incorrect")
                                filter_idx = torch.tensor(filter_all_incorrect_list, dtype=torch.bool)
                            batch.subset(filter_idx)

                        if batch.batch.batch_size[0] > 0:
                            collected_batches.append(batch)
                            print(f"current batch number {DataProto.concat(collected_batches).batch.batch_size[0]}")
            if len(collected_batches) == 0:
                continue
            print("filter successful, ready to training.")
            batch = DataProto.concat(collected_batches)

            if batch.batch.batch_size[0] > target_batch_size:
                remained_batch = DataProto.concat(collected_batches)
                remained_batch_mask = torch.tensor([False] * target_batch_size + [True] * (batch.batch.batch_size[0] - target_batch_size), dtype=torch.bool)
                batch_mask = torch.tensor([True] * target_batch_size + [False] * (batch.batch.batch_size[0] - target_batch_size), dtype=torch.bool)
                remained_batch.subset(remained_batch_mask)
                batch.subset(batch_mask)
            ...
            batch = compute_advantage(batch,
                                        ...
                                        max_reward=self.config.algorithm.max_reward,
                                        add_max_reward_num=self.config.algorithm.add_max_reward_num
                                        )
            # update critic
            if self.use_critic:
