...
def compute_ngrpo_outcome_advantage(token_level_rewards: torch.Tensor,
                                   eos_mask: torch.Tensor,
                                   index: torch.Tensor,
                                   epsilon: float = 1e-6,
    ...
    Args:
        ...
        eos_mask: `(torch.Tensor)`
            shape: (bs, response_length)
        max_reward: `float`
            the setting of max reward for the task
    ...
    with torch.no_grad():
        ...
        for idx in id2score:
            if len(id2score[idx]) == 1:
                ...
                id2mean[idx] = torch.mean(torch.cat([torch.tensor(id2score[idx]), torch.tensor([max_reward] * add_max_reward_num)]))
                id2std[idx] = torch.std(torch.cat([torch.tensor(id2score[idx]), torch.tensor([max_reward] * add_max_reward_num)]))
            else:
...
def compute_policy_loss(old_log_prob, log_prob, advantages, eos_mask, cliprange, token_level_scores, positive_learning_weight=None, cliprange_low=None, cliprange_high=None):
